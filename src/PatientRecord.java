...
public class PatientRecord implements Comparable<PatientRecord> {
    // data fields
    private static int patientCounter = 1; // counts the number of patients created, Begins at
    // 1, and advances to next value after each caseID is generated.
     ...
     *
     * The first digit of the case is based on gender marker: F=1, M=2, X=3. Any other gender
     * marker should be assigned the first digit of 4. The next two digits of the patient's age:
     * 03 could mean a three-year-old or a 103-year-old The last two digits increment according
     * to the number of patients admitted during this run of ExceptionalCare; the first patient
     * should be 01, counting up to 99, and then wrapping around to 00.
     *
     ...
     * should be called at the beginning of EACH tester method to ensure that the methods are not
     * dependent on being called in a particular order.
     */
     ...
     * other have the exact same CASE_Number
     */
    @Override
     ...
     * order of arrival. For example, if PatientRecord A arrived before PatientRecord B and both
     * have a YELLOW triage level, then A is less than B.
     *
     * If the PatientRecords have the same triage level and the same order of arrival, then they
     * are considered equal. (This is expected behavior of PatientRecord even though you will not
     * have equal PatientRecords in your priority queue.)
     *
    ...
    public int compareTo(PatientRecord other) {
        ...
        if(triage.compareTo(other.triage) < 0){
            return -1;
        }

        else if(triage.compareTo(other.triage) > 0){
            return 1;
        }

        else if(triage.equals(other.triage)){
            if(getArrivalOrder() < other.getArrivalOrder()){
                return -1;
            }

            else if(getArrivalOrder() > other.getArrivalOrder()){
                return 1;
            }
        }
